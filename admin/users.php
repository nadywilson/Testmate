...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM assigned_quizzes WHERE user_id = $del_id");
...
if ($view_user > 0) {
    ...
    $uf = $conn->prepare("
        SELECT fq.*, q.question, q.option_a, q.option_b, q.option_c, q.option_d, q.correct_answer, t.name AS topic_name
    ...
    $ua = $conn->prepare("
        SELECT aq.*, q.question AS question_text
        FROM assigned_quizzes aq
        JOIN questions q ON aq.question_id = q.id
        WHERE aq.user_id = ?
        ORDER BY aq.assigned_at DESC
    ");
    $ua->bind_param("i", $view_user);
    $ua->execute();
    $user_assigned = $ua->get_result()->fetch_all(MYSQLI_ASSOC);
}

$users = $conn->query("
    SELECT u.id, u.name, u.email, u.role, u.created_at, u.licence_code,
           COUNT(DISTINCT qs.id) AS quizzes,
           COUNT(DISTINCT ms.id) AS mocks,
           COUNT(DISTINCT fq.id) AS failed_q,
           COUNT(DISTINCT aq.id) AS assigned
    FROM users u
    ...
    LEFT JOIN assigned_quizzes aq ON u.id = aq.user_id AND aq.is_completed = 0
    GROUP BY u.id
...
<head>
    ...
    <style>
        ...
        .pending-tag{background:#eaf4ff;color:#2471a3;padding:3px 10px;border-radius:20px;font-size:12px;font-weight:600;}
    </style>
</head>
<body>
<nav class="navbar">
...
<div class="admin-wrap">
    ...
    <div class="main-content">
        ...
        <!-- Mock Test History -->
        <h2 style="font-size:18px;font-weight:700;margin-bottom:14px;">Mock Test History</h2>
        <?php if (empty($user_mocks)): ?>
        <div class="card" style="text-align:center;padding:24px;color:#888;">No mock tests taken yet.</div>
        <?php else: ?>
        ...
        <div class="table-wrap">
            <table>
                <thead>
                    <tr><th>Name</th><th>Email</th><th>Code</th><th>Role</th><th>Quizzes</th><th>Tests</th><th>Failed</th><th>Pending</th><th>Joined</th><th>Actions</th></tr>
                </thead>
                <tbody>
